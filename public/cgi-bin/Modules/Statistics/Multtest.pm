...
sub initial {
    ...
    if(! Modules::List::Vectorize::is_array_ref($p) and ! Modules::List::Vectorize::is_hash_ref($p)) {
        croak "ERROR: P-values should be stored in an array reference or a hash reference.\n";
    }
    
    my $name = [];
    if(Modules::List::Vectorize::is_hash_ref($p)) {
        $name = [ keys %{$p} ];
        $p = [ values %{$p} ];
    }
...
sub _qvalue {
    ...
    if(! Modules::List::Vectorize::is_array_ref($lambda)) {
...
directly from R to Perl using L<Modules::List::Vectorize> module.
