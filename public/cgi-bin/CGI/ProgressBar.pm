...
sub progress_bar {
	...
	my $pb = bless {
		_updates=> 0,		debug	=> undef, mycss=>'',
		orientation	=> 'horizontal',
		from	=> 1,		to	=> 100,	width	=> '400',
		height	=> '20',	blocks	=> 10,
		label	=> 0,		colors	=> [10,'blue']
	},__PACKAGE__;
...
#	$pb->{_interval} = $pb->{_length}>0? ($pb->{_length}/$pb->{blocks}) : 0;
	...
	return $self->_pb_init($self->{progress_bar});
}
...
sub hide_progress_bar {
	my ($self, @crud) = CGI::self_or_default(@_);
...
sub CGI::_pb_init {
	my ($self, @crud) = CGI::self_or_default(@_);
	#$self->{progress_bar}=shift;
	my $html = "";
	#warn 'New CGI::ProgressBar  dump '.Dumper($pb);

	#warn 'New CGI::ProgressBar  dump '.Dumper(@_);

	#warn 'New CGI::ProgressBar  dump '.Dumper($self->{progress_bar});
	...
	$self->CGI::_init_css;
	my $my_css = ($self->{progress_bar}->{mycss}) ? $self->{progress_bar}->{mycss} : '';
	my $this_css = ($self->{progress_bar}->{css}) ? $self->{progress_bar}->{css} : '';
	$html .= "<style type='text/css'>$this_css\n$my_css</style>\n";
	...
	foreach my $i (1 .. $self->{progress_bar}->{blocks}){$html .= "<span class='pblib_block_off' id='$self->{progress_bar}->{layer_id}->{block}$i'>&nbsp;</span>";}
	$html .= "\n\t</div>\n";
	$html .= "</td></tr>\n<tr><td align='center'>
...
sub CGI::_init_css {
	my ($self, @crud) = CGI::self_or_default;
	my $pblibBarWidth = ($self->{progress_bar}->{width}) ? "width: ".($self->{progress_bar}->{width})."px;" : '';
	my $css1;
	if($self->{progress_bar}->{orientation}){
		my $height = ($self->{progress_bar}->{block_wi}) ? "height: ".($self->{progress_bar}->{block_wi})."px;" : '';
		my $width = ($self->{progress_bar}->{block_wi}) ? "width: ".($self->{progress_bar}->{block_wi})."px;" : '';
		$css1 = $self->{progress_bar}->{orientation} eq 'vertical' ?
			"float:none;
			 width: 100%;
			 $height"
		  : "float:left;
